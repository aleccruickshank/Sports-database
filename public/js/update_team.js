...
let updatePlayerForm = document.getElementById('update-team-form-ajax');
...
updatePlayerForm.addEventListener("submit", function (e) {
    ...
    let inputName = document.getElementById("input-team-update");
...
function updateRow(data, team_id){
    ...
    for (let i = 0, row; row = table.rows[i]; i++) {
        ...
        if (table.rows[i].getAttribute("data-value") == team_id) {
            ...
            td.innerHTML = parsedData[0].team_id;
            location.reload();
        }
    }
}